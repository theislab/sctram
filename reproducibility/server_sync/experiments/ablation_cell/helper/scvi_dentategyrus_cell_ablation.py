...
from ablation_cell.helper.random_remove import remove_random_fraction_from_labels
...
def main():
    ...
    parser.add_argument("--labels_to_affect", type=str, required=True)
    parser.add_argument("--seed", type=int, required=True)
    
    args = parser.parse_args()
    fraction = args.fraction
    seed = args.seed
    labels_to_affect = args.labels_to_affect.split("|")
    ...
    if fraction is None:
        adata = adata[~adata.obs["clusters"].isin(remove_labels)].copy()
    else:
        adata = remove_random_fraction_from_labels(adata, k=fraction, seed=seed, cluster_col="clusters", labels_to_affect=labels_to_affect)
    ...
    model_dir_path = os.path.join(dataset_dir, f"scvi_dentategyrus_ablation_cell_{args.labels_to_affect}_fraction_{fraction}_seed{seed}")
    vae.save(
        model_dir_path,
        overwrite=True,
    )
    ...
    latent_dir_path = os.path.join(dataset_dir, f"scvi_dentategyrus_ablation_cell_{args.labels_to_affect}_fraction_{fraction}_seed{seed}.h5ad")
