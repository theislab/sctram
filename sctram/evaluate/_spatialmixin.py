...
class SpatialMetricsMixin:
    ...
    def compute_spatial_weights(
        self, data: Any, input_type: str, **kwargs
    ) -> csr_matrix:
        """
        ...
        if input_type == 'adjacency':
            ...
            if weight_method == 'inverse':
                distance_matrix = np.abs(
                    pseudotime[:, np.newaxis] - pseudotime[np.newaxis, :]
                )
                ...
                raise ValueError(f"Unknown weight_method '{weight_method}' for pseudotime.")
            ...
            if weight_method == 'knn':
                ...
                spatial_weights = kneighbors_graph(
                    embedding, n_neighbors=k, mode='connectivity', include_self=True
                )
                ...
                raise ValueError(f"Unknown weight_method '{weight_method}' for diffmap.")
            ...
            raise ValueError(
                f"Unsupported input_type '{input_type}'. Supported types: 'adjacency', 'pseudotime', 'diffmap'."
            )
