...
def _calculate_path_curvature(
    ...
    for t_val in np.linspace(0, 1, n_samples):
        ...
        numerator = np.sqrt(max(  # Ensure non-negative under floating point errors
            (norm_double_prime**2 * norm_prime**2) - dot_product**2,
            0
        ))
...
if __name__ == "__main__":
    ...
    def test_linear_path_2d():
        ...
        score = normalized_mean_curvature(
            G, centroids, validate_result=True, 
            min_path_length=3, n_sample_points=1000, aggregation_method='median'
        )
        assert np.isclose(score, 0.0, atol=1e-4), \
            f"Linear path should have near-zero score. Got {score}"
    ...
    def test_semicircle_geometry():
        ...
        score = normalized_mean_curvature(
            G, centroids, validate_result=False,  # Disable validation for precise check
            min_path_length=3, n_sample_points=5000, aggregation_method='mean'
        )
        
        expected = (1.0 / np.pi)  # κ=1 for r=1, normalized by π
        assert np.isclose(score, expected, rtol=0.05), \
            f"Semicircle κ should be ~{expected:.3f}. Got {score:.3f}"
    ...
    def test_aggregation_consistency():
        ...
        assert np.isclose(score_mean, np.mean(test_curvatures)), \
            f"Mean fail: {score_mean}≠{np.mean(test_curvatures)}"
    ...
    def test_high_dimension_stability():
        ...
        score = normalized_mean_curvature(
            G, centroids, validate_result=True,
            min_path_length=3, n_sample_points=1000
        )
        assert np.isclose(score, 0.0, atol=1e-4), \
            f"High-dim linear path score invalid: {score}"
    ...
    def test_short_path_handling():
        ...
        with pytest.raises(ValueError, match="no valid paths"):
            normalized_mean_curvature(
                G, centroids, validate_result=False, min_path_length=4
            )
    ...
    def test_validation_trigger():
        ...
        class MockGraph:
            def has_path(*args): return True
        def mock_curvature(*args, **kwargs): return 1.5  # Invalid score
        ...
        with pytest.raises(ValueError, match="must be between 0 and 1"):
            normalized_mean_curvature(
                nx.DiGraph([('A','B')]), Centroids(), validate_result=True
            )
